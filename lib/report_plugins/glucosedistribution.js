...
glucosedistribution.report = function report_glucosedistribution(datastorage, sorteddaystoshow, options) {
    ...
    // add TooLow, TooHigh
    var colors = ['#f33', '#f88', '#8f8', '#ff8', '#ff3' ];
    var tablecolors = {
        TooLow: '#f33',
        Low: '#f88',
        Normal: '#8f8',
    ...
    var dsdata = datastorage.devicestatus;
    console.log(dsdata);
    var totalInsulin = dsdata[0].pump.reservoir - dsdata[dsdata.length-1].pump.reservoir;
...
//     var prevEntry = glucose_data[i-1];
   ...
   glucose_data = data = glucose_data2.filter(function(r) {
    ...
    // add TooLow, TooHigh
    ['TooLow', 'Low', 'Normal', 'High', 'TooHigh' ].forEach(function(range) {
        result[range] = {};
        var r = result[range];
	// 60, 80, 180, 200
        r.rangeRecords = glucose_data.filter(function(r) {
            if (range === 'TooLow') {
                return r.sgv > 0 && r.sgv <= 60;
            } else if (range === 'Low') {
                return r.sgv >= 60 && r.sgv <= options.targetLow;
            } else if (range === 'Normal') {
                return r.sgv > options.targetLow && r.sgv <= options.targetHigh;
            } else if (range === 'High') {
                return r.sgv > options.targetHigh && r.sgv <= 200;
            } else {
                return r.sgv > 200;
            }
        });
    ...
    result.Normal.readingspct = (100 - result.Low.readingspct - result.High.readingspct - result.TooLow.readingspct - result.TooHigh.readingspct ).toFixed(1);

    ['TooHigh', 'High', 'Normal', 'Low', 'TooLow' ].forEach(function(range) {
    //['TooLow', 'Low', 'Normal', 'High', 'TooHigh' ].forEach(function(range) {
        var tr = $('<tr>');
        var r = result[range];
...
//            console.log("Record skipped");
    ...
    console.log('glucoseTotal', glucoseTotal, 'glucoseMean', glucoseMean,'tirMultiplier',tirMultiplier, 'PGS',PGS);
...
//	console.log('TADC',TDC,'days',days);
    ...
    $('<tr><th>Total Insulins from DeviceStatus</th><th>Starting Reservoir</th><th>Ending Reservoir</th></tr>').appendTo(stabilitytable);
    console.log("DeviceStatus: ", dsdata[0].pump.reservoir, dsdata.length, totalInsulin);
    $('<tr><td class="tdborder">' + totalInsulin +'</td><td class="tdborder">' + dsdata[0].pump.reservoir + '</td><td class="tdborder">'+ dsdata[dsdata.length-1].pump.reservoir + '</td></tr>').appendTo(stabilitytable);
