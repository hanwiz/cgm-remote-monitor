...
dailystats.report = function report_dailystats (datastorage, sorteddaystoshow, options) {
  ...
  sorteddaystoshow.forEach(function(day) {
    ...
    $('<td class="tdborder" style="width:160px">' + report_plugins.utils.localeDate(day) + '</td>').appendTo(tr);
    $('<td class="tdborder">' + Math.round((100 * stats.toolows) / daysRecords.length) + '%</td>').appendTo(tr);
    ...
    $('<td class="tdborder">' + Math.round((100 * stats.toohighs) / daysRecords.length) + '%</td>').appendTo(tr);
    ...
    var inrange = [
      {
        label: translate('TooLow')
        , data: Math.round(stats.toolows * 1000 / daysRecords.length) / 10
      }
      , {
        label: translate('Low')
      ...
      , {
        label: translate('TooHigh')
        , data: Math.round(stats.toohighs * 1000 / daysRecords.length) / 10
      }
    ];
    $.plot(
